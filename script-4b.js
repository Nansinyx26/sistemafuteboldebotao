...
// CHAVE ÚNICA PARA TURMA 4B
const STORAGE_KEY = 'dados_campeonato_4B';
...
function saveGameData() {
    try {
        ...
        console.log('✅ Dados da Turma 4B salvos com sucesso!');
...
function loadGameData() {
    try {
        ...
        if (savedData) {
            ...
            if (gameData.games && gameData.scorers && gameData.standings && gameData.turma === '4B') {
                games = gameData.games;
                scorers = gameData.scorers;
                standings = gameData.standings;
                currentGameIndex = gameData.currentGameIndex || 0;

                console.log('✅ Dados da Turma 4B carregados com sucesso!');
                return true;
            }
        }
    } catch (error) {
...
function resetTournament() {
    if (confirm('⚠️ Tem certeza que deseja resetar todo o campeonato da Turma 4B?\n\nTodos os jogos, gols e classificações serão perdidos!')) {
        localStorage.removeItem(STORAGE_KEY);
        currentGameIndex = 0;
        scorers = {};
        ...
        showNotification('🔄 Campeonato da Turma 4B resetado com sucesso!');
    }
}

// Inicialização
function init() {
    console.log('🚀 Inicializando Turma 4B...');
    const dataLoaded = loadGameData();

    if (!dataLoaded) {
        console.log('📝 Criando novos dados para Turma 4B...');
        initializeGames();
        initializeStandings();
    ...
    if (dataLoaded) {
        ...
        if (finishedGames > 0) {
            showNotification(`✅ Turma 4B: ${finishedGames} jogos carregados`);
        }
    }
}
...
function showTournamentComplete() {
    ...
    document.getElementById('gameSheet').innerHTML = `
        <div class="final-message">
            <h3>Parabéns a todos os participantes da Turma 4B!</h3>
    ...
    container.innerHTML = `
        <div class="fifa-bracket">
            <div class="fifa-header">
                <h3>🏆 ARTILHARIA</h3>
                <p>CAMPEONATO DE FUTEBOL 2025 - TURMA 4B</p>
